...
mtadata_bin <- left_join(
    mtdata,
    binning,
    by = c("new_contig" = "contig"),
    multiple = "all"
) %>%
  mutate(
    bin_name = if_else(is.na(bin_name), new_contig, bin_name),
    bin_name = as.character(bin_name),
    type = if_else(prophage == "yes", "lysogenic", type)
  ) %>%
  group_by(bin_name) %>%
  reframe(
    type = case_when(
  length(type) == 0 ~ NA_character_,
  all(is.na(type)) ~ NA_character_,
  "lysogenic" %in% type ~ "lysogenic",
  TRUE ~ first(na.omit(type))
),
    prophage = if_else("yes" %in% prophage, "Yes", "No")
  )
...
metadata_final<- full_join(quality, metadata_trim, by=c("contig_id"="contig")) %>%
  select(contig_id, contig_length, prophage, type, gene_count,
         viral_genes, host_genes, checkv_quality,warnings,completeness_method, retained, reason) %>%
  mutate(
  prophage = ifelse(is.na(prophage), "No", prophage),
  type = case_when(
    prophage == "Yes" ~ "lysogenic",
    is.na(type) ~ "lytic",
    TRUE ~ type
  ),
  sample = sam
