...
def find_unmap_paired_files(data_dir):
    """递归查找并配对 *_unmap_1.fq.gz 和 *_unmap_2.fq.gz 文件"""
    file_groups = defaultdict(dict)
    pattern = r'^(.+)_unmap_([12])\.fq\.gz$'
...
def main():
    parser = argparse.ArgumentParser(description="Generate metadata for unmap fq.gz pairs")
    ...
    file_groups = find_unmap_paired_files(args.data_dir)
    records = []
    ...
    output_path = os.path.join(output_dir, f"sample_metadata_{args.bioproject}_unmap.tsv")

    df = pd.DataFrame(records)
    df.to_csv(output_path, sep="\t", index=False)

    print(f"[INFO] Metadata file written to: {output_path}")
    print(f"[INFO] Found {len(records)} valid unmap sample pairs")
